#!/usr/bin/env bash
#
# Build an iOS App Store archive for Acacia and optionally upload it to App Store Connect.
#
# Usage:
#   scripts/build-ios-app-store-archive.sh [--version VERSION] [--build-number NUMBER] [--upload] [--skip-archive]
#
set -euo pipefail
...
OUTPUT_DIR="${APP_STORE_IOS_DIST_DIR:-$ROOT_DIR/dist/app-store-ios}"
ARCHIVE_PATH_OVERRIDE="${ARCHIVE_PATH:-}"
EXPORT_PATH_OVERRIDE="${EXPORT_PATH:-}"
ARCHIVE_PATH=""
EXPORT_PATH=""
WORKSPACE="${WORKSPACE:-$ROOT_DIR/ios/PDFViewer.xcworkspace}"
SCHEME="${SCHEME:-PDFViewer}"
UPLOAD=0
...
if [[ -z "$ARCHIVE_PATH" ]]; then
  ARCHIVE_PATH="${ARCHIVE_PATH_OVERRIDE:-$OUTPUT_DIR/Acacia-iOS-${VERSION}-${BUILD_NUMBER}.xcarchive}"
fi
...
if [[ ! -d "$WORKSPACE" ]]; then
  echo "Missing iOS workspace: $WORKSPACE" >&2
  echo "Run: pod install --project-directory=ios" >&2
  exit 1
fi
...
repair_react_native_resource_bundles() {
  ...
  while IFS= read -r -d '' bundle_path; do
    if [[ -d "$bundle_path/_CodeSignature" || -d "$bundle_path/Contents/_CodeSignature" ]]; then
      /usr/bin/codesign --remove-signature "$bundle_path" || true
      repaired=$((repaired + 1))
    fi
  done < <(/usr/bin/find "$app_path" -maxdepth 2 -type d -name '*.bundle' -print0)
...
EXPORT_OPTIONS_PLIST="$(mktemp "${TMPDIR:-/tmp}/AcaciaIOSAppStoreExportOptions.XXXXXX")"
DESTINATION="export"
if [[ "$UPLOAD" == "1" ]]; then
  DESTINATION="upload"
fi
...
echo "=== Acacia iOS App Store archive ==="
...
if [[ "$SKIP_ARCHIVE" != "1" ]]; then
  ARCHIVE_ARGS=(
    ...
    "ACACIA_FIREBASE_WEB_API_KEY=${ACACIA_FIREBASE_WEB_API_KEY:-}"
    "ACACIA_PRO_API_BASE_URL=${ACACIA_PRO_API_BASE_URL:-}"
...
ensure_archive_dsym "$ARCHIVE_PATH" "$ARCHIVE_PATH/Products/Applications/Acacia.app/Acacia" "Acacia.app.dSYM"
ensure_archive_dsym "$ARCHIVE_PATH" "$ARCHIVE_PATH/Products/Applications/Acacia.app/Frameworks/hermes.framework/hermes" "hermes.framework.dSYM"
...
echo ""
echo "iOS App Store archive complete:"
echo "  $ARCHIVE_PATH"
echo "  $EXPORT_PATH"
echo "  $METADATA_PATH"
